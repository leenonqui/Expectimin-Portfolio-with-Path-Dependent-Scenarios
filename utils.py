...
def calculate_real_return(nominal_return: float, inflation: float) -> float:
    ...
    """
    return ((1 + nominal_return/100) / (1 + inflation/100) - 1) * 100

def get_risk_free_rate(data: pd.DataFrame, prediction_year: int) -> float:
    """
    ...
    """
    if prediction_year not in data.index:
        # If prediction year not available, use last available year
        last_year = data.index.max()
        if last_year < prediction_year:
            print(f"Warning: Using {last_year} data for risk-free rate (latest available)")
            prediction_year = last_year
        else:
            raise ValueError(f"Year {prediction_year} not in data")
    
    # Get nominal bond rate and inflation for the year
    nominal_bond_rate = data.loc[prediction_year, 'bond_tr']
    inflation_rate = data.loc[prediction_year, 'inflation']
    
    # Calculate real rate
    real_rate = calculate_real_return(nominal_bond_rate, inflation_rate)
